...
def get_font(name: str, size: int, bold: bool = False, italic: bool = False):
    """
    Get a pygame font with fallback support for WASM/web environments.
    
    Args:
        name: Preferred font name (will fall back if not available)
    ...
    # Try to get system font with preferred name
    try:
    ...
    fallbacks = ['arial', 'helvetica', 'sans-serif', 'freesans', 'liberationsans']
    
    for fallback_name in fallbacks:
        try:
            font = pygame.font.SysFont(fallback_name, size, bold=bold, italic=italic)
            if font:
    ...
    print(f"Using pygame default font (size {size})")
